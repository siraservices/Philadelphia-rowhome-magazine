<?php
/**
 * Section landing body — shared by template-section.php (pages) and
 * taxonomy-department_category.php (term archives).
 *
 * Expects query var `rh_section_term` (WP_Term|false). Falls back to the
 * queried object when it is a department_category term.
 *
 * @package RowHome_Magazine
 * @since 2.1.0
 */

$section_term = get_query_var( 'rh_section_term', null );
if ( $section_term === null ) {
...
<!-- Section Hero Banner -->
<?php $section_page_id = is_page() ? get_queried_object_id() : 0; ?>
<div class="section-hero<?php echo ( $section_page_id && has_post_thumbnail( $section_page_id ) ) ? ' section-hero--with-bg' : ''; ?>"
     <?php if ( $section_page_id && has_post_thumbnail( $section_page_id ) ) : ?>style="background-image: url('<?php echo esc_url( get_the_post_thumbnail_url( $section_page_id, 'full' ) ); ?>');"<?php endif; ?>>
    <div class="container">
